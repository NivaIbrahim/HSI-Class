...
```{r}
library(stars)
HSI_raw<- read_stars("../data/HSI_Raw.tif")
...
```{r}
HSI_Seg <- rast("../data/HSI_Segmented.tif")
...
```{r}
Red_Band <- raster(HSI_Seg$Band_114) #Assuming band 114 is Red (650 nm)
...
#writeRaster(NDVI, "../output/NDVI.tif")
```
...
```{r}
sampling_pts <-st_read("../data/Plots/Taft7_Sampling_Points.shp") 

sampling_pts 
```

```{r}
plot_buffer<- st_read("../data/Plots/Taft7_Sampling_Points.shp") %>%
  st_transform(crs = 32617) %>%
  st_buffer(dist = 10) %>%   #buffer by 10m
...
#st_write(plot_buffer, "../output/plot_buffer.shp")  #write_sf
```
...
```{r}
avg_spectra <- terra::extract(HSI_Seg,circle,'mean', na.rm= TRUE)
avg_spectra
```

```{r}
# Ensure the length of the sequence matches the number of rows in the data frame
avg_spectra$ID <- 701:(701 + nrow(avg_spectra) - 1)
...
afl<- read.csv("../data/Taft7_Aflatoxin_Results_2023.csv")
afl
...
# rename the columns
afl_clean <- afl %>%
  rename(Plot= Plot., afl_0809 = X8.9.2023)
afl_clean
```

```{r}
afl_spectra<- afl_clean %>%
  left_join(avg_spectra) 
...
-   workflow of Headwall HSI -Segmentation process -Single Band Extraction -Vegetation Indices Calculation -Reflectance Values Extraction
-   
